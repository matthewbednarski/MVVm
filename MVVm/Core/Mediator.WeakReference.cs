...
namespace MVVm.Core
{
	...
	{
		public static readonly String DefaultKey = "_any";
		...
		{
			get{
				if(_objectActionType == null)
				{
					object o = new object();
					_objectActionType = typeof(Action<>).MakeGenericType(o.GetType());
				}
				return _objectActionType;
			}
		}
		...
		{
			get{
				if(_stringActionType == null)
				{
					string o = "";
					_stringActionType = typeof(Action<>).MakeGenericType(o.GetType());
				}
				return _stringActionType;
			}
		}
		#endregion
		...
		private void RegisterHandler(object key, Type actionType, Delegate handler)
		{
			...
			lock (_registeredHandlers)
			{
				...
				if (_registeredHandlers.TryGetValue(key, out wr))
				{
					...
					{
						WeakAction wa = wr[0];
						if (wa.ActionType != actionType && (!wa.ActionType.IsAssignableFrom(actionType) && !actionType.IsAssignableFrom(wa.ActionType)))
						{
							if((wa.ActionType == ObjectActionType || actionType == ObjectActionType))
							{
		...
		/// </summary>
		/// <param name="key">Message key: usually a string</param>
		...
		private bool NotifyColleagues(object key, object message)
		{
			...
			Type messageActionType = typeof(Action<>).MakeGenericType(message.GetType());
			foreach (var cb in wrCopy)
			{
				Delegate action = cb.GetMethod();
				Type actionType = cb.ActionType;
				if (action != null)
				{
					if (messageActionType != actionType && (!messageActionType.IsAssignableFrom(actionType) && !actionType.IsAssignableFrom(messageActionType)))
					{
						if((messageActionType == ObjectActionType || actionType == ObjectActionType))
						{
							action.DynamicInvoke(message);
						}else{
							action.DynamicInvoke(message.ToString());
						}
					}else{
						action.DynamicInvoke(message);
					}
		...
		public bool NotifyColleagues<T>(T message)
		{
			...
			if(rc == false)
			{
				rc |= this.NotifyColleagues(Mediator.DefaultKey, message);
			}
			return rc;
		}
	...
	public sealed class MediatorMessageSinkAttribute : Attribute
	{
		...
		{
			MessageKey = MVVm.Core.Mediator.DefaultKey;
		}
